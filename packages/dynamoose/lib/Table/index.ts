...
import {createTable, createTableRequest, pointInTimeRecoveryEngaged, updatePointInTimeRecovery, updateTable, updateTimeToLive, waitForActive} from "./utilities";
...
/**
 * Injects a table's TTL (`expires`) attribute into the given models' schemas. Dynamoose only persists
 * attributes its schema knows about, so a schema that never receives this injection silently drops the
 * TTL attribute on every write while DynamoDB keeps reporting `TimeToLiveStatus: ENABLED`.
 *
 * Called both from the `Table` constructor and, for a model registered against an already-constructed
 * table, from `Model`'s attach path.
 */
export function applyExpiresToSchemas (models: any[], expires: TableExpiresSettings): void {
	utils.array_flatten(models.map((model: any) => model.Model.getInternalProperties(internalProperties).schemas)).forEach((schema) => {
		// `setSchemaAttribute` also invalidates the schema's memoization caches, which would otherwise keep
		// serving a stale attribute view that omits this attribute.
		schema.getInternalProperties(internalProperties).setSchemaAttribute(expires.attribute, {
			"type": {
				"value": Date,
				"settings": {
					"storage": "seconds"
...
export class Table extends InternalPropertiesClass<TableInternalProperties> {
	 ...
	 * | update | If Dynamoose should update the capacity of the existing table to match the model throughput. If this is a boolean of `true` all update actions will be run. If this is an array of strings, only the actions in the array will be run. The array of strings can include the following settings to update, `ttl`, `indexes`, `throughput`, `tags`, `tableClass`, `streams`, `deletionProtection`, `pointInTimeRecovery`. Note: `indexes` deletes and recreates a global secondary index whose key schema or projection changed, so that index is unavailable until DynamoDB finishes backfilling it. | Boolean \| [String] | false |
	 ...
	 * | deletionProtection | If Dynamoose should enable deletion protection on the table, blocking `DeleteTable` until disabled. Note: with `update` set to `true` this can only enable deletion protection; disabling requires `update` to explicitly include `"deletionProtection"`. | Boolean | false |
	...
	constructor (instance: Instance, name: string, models: Model[], options: TableOptionsOptional = {}) {
		...
		this.setInternalProperties(internalProperties, {
			...
			"runSetupFlow": async (): Promise<void> => {
				...
				if (pointInTimeRecoveryEngaged(this)) {
