...
|------------------|----------------|-------|
| new Table() | `createTable`, `describeTable`, `updateTable`, `updateTimeToLive`, `describeTimeToLive`, `listTagsOfResource`, `tagResource`, `untagResource`, `describeContinuousBackups`, `updateContinuousBackups` | `createTable` is only used if `create` is set to true. `describeTable` is only used if `waitForActive` OR `create` is set to true. `updateTable` is only used if `update` is set to true. `updateTimeToLive` & `describeTimeToLive` is only used if `create` or `update` is set to true and there is an `expires` setting on the model. `listTagsOfResource`, `tagResource`, `untagResource` are only used if `update` is set to true or includes `tags` in the array of strings. `describeContinuousBackups` & `updateContinuousBackups` are only used for point-in-time recovery: on `create` when `pointInTimeRecovery.enabled` is true, when `update` includes `pointInTimeRecovery` (the only way to disable it), or when `update` is true and `pointInTimeRecovery.enabled` is true. Deletion protection uses `updateTable`, only when `update` includes `deletionProtection` (the only way to disable it) or when `update` is true and `deletionProtection` is true. |
